(() => {
    ...
    class Loader {
        ...
        define(id, dependencyIds, callback) {
            ...
            // extending in ES modules transpiles to AMD `define`, we need to override th first dependency
            if (id in this._extensionsDependencyGraph) {
                this._extending = true;

                dependencyIds[0] = this._extensionsDependencyGraph[id];
            }
         ...
         *   passed as parameters. Should return a value to define the module.
         */
        extend(id, dependencyIds, callback) {
            id = this._getId(id);
         ...
         * @private
         */
        _define(id, dependencyIds, callback) {
            if (!dependencyIds) {
            ...
            if (Array.isArray(dependencyIds)) {
                dependencyIds = dependencyIds.map(depId =>
                    this._normalizeIdPath(depId, id),
                );
            }
            ...
            promise.finally(() => {
                delete this._definePromiseMap[id];
            });

            this._definePromiseMap[id] = promise;
        }
        ...
        _normalizeIdPath(id, subjectId) {
            ...
            if (up) {
                outputPath =
                    dirParts.slice(0, -up).join('/') +
                    '/' +
                    outputPath.slice(3 * up);
            }
        ...
        _normalizeId(id) {
            ...
            if (
                ...
                console.warn(
                    `${id}: module ID should use slashes instead of dots and hyphen instead of CamelCase.`,
                );
            }
         ...
         */
        async _load(id, callback, errorCallback) {
            if (id === 'exports') {
                callback({});
            ...
            if (id in this.viewExtensionMap && !this._extending) {
                id = this.viewExtensionMap[id].slice(-1)[0];
            }
            ...
            if (id.indexOf('lib!') === 0) {
                ...
                if (id in this._definePromiseMap) {
                    await this._definePromiseMap[id];
                ...
                if (restoredId in this._bundleMapping) {
                    ...
                    await this._requireBundle(bundleName);
        ...
        addLibsConfig(data) {
            if (
                this._addLibsConfigCallCount === this._addLibsConfigCallMaxCount
            ) {
    ...
    (() => {
        const loaderParamsTag = document.querySelector(
            'script[data-name="loader-params"]',
        );
    ...
    Object.defineProperty(root, 'Espo', {
        ...
        set: value => {
            ...
            /**
             * loader.js requires `Espo.loader` to be unset, otherwise it throws
             */
            let _isSet = false;
            Object.defineProperty(_espo, 'loader', {
                get: () => (_isSet ? _loader : undefined),
                set: () => {
                    _isSet = true;
                },
                configurable: true,
